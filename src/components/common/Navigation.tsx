...
const Navigation: React.FC = () => {
  ...
  const closeMobileMenu = () => {
    ...
    addTimeout(() => {
      ...
      // restore focus
      (lastFocusRef.current ?? menuBtnRef.current)?.focus();
  ...
  const handleMenuEnter = useCallback(() => {
    ...
    addTimeout(() => isHoveringRef.current && setEnterStage('expandingDown'), prefersReduced ? 0 : 200);
    addTimeout(() => isHoveringRef.current && setEnterStage('open'), prefersReduced ? 0 : 400);
  }, [mobileMode, isMenuOpen, enterStage, addTimeout, clearAllTimeouts, prefersReduced]);
  ...
  const handleMenuLeave = useCallback(() => {
    ...
    clearAllTimeouts();
    setIsExiting(true);
    addTimeout(() => !isHoveringRef.current && setExitStage('collapsingUp'), prefersReduced ? 0 : 300);
    addTimeout(() => !isHoveringRef.current && setExitStage('collapsingLeft'), prefersReduced ? 0 : 500);
    addTimeout(() => {
  ...
  }, [mobileMode, isMenuOpen, addTimeout, clearAllTimeouts, prefersReduced]);
  ...
  useEffect(() => {
    animate(progress, isDark ? 1 : 0, { duration: prefersReduced ? 0 : 0.42, ease: easeOut });
  }, [isDark, progress, prefersReduced]);
    ...
    <>
          ...
          <div className="navbar-grid">
                    ...
                    {backMode ? (
                      ...
                      <>
                        <motion.img
                          ...
                          transition={{ duration: prefersReduced ? 0 : 0.25, ease: 'easeInOut', opacity: { duration: prefersReduced ? 0 : 0.15 } }}
                        />
                        <motion.img
                          ...
                          transition={{ duration: prefersReduced ? 0 : 0.25, ease: 'easeInOut', opacity: { duration: prefersReduced ? 0 : 0.15, delay: mobileMode && isMenuOpen && !isClosingIcon ? 0.1 : 0 } }}
                        />
                      </>
                    )}
                  </div>
            ...
            <motion.div className="toggle-anchor" style={{ x: toggleX, y: toggleY, display: mobileMode && isMenuOpen ? 'none' : 'flex' }}>
              <motion.button
                ...
                transition={{ duration: prefersReduced ? 0 : 0.35, ease: easeOut }}
              >
                <motion.div className="switch-visual" animate={{ width: switchVisualW }} transition={{ duration: prefersReduced ? 0 : 0.28, ease: easeOut }}>
                  {mobileMode ? (
                    <div className="switch-mobile-face">
                      <div className="switch-flip-24">
                        <motion.div initial={false} animate={{ rotateY: isDark ? 0 : 180 }} transition={{ type: 'tween', duration: prefersReduced ? 0 : 0.42, ease: easeOut }} className="flip-3d">
                    ...
                    <div className="switch-track">
                      <motion.div className="switch-fill" style={{ left: fillLeftPx }}>
                        <div className="switch-fill-center">
                          <motion.div initial={false} className="flip-3d" style={{ rotateY }} transition={{ type: 'tween', duration: prefersReduced ? 0 : 0.42, ease: easeOut }}>
                            <img src="/images/moon.svg" alt="" aria-hidden="true" className="flip-face front" />
                            <img src="/images/sun.svg" alt="" aria-hidden="true" className="flip-face back" />
                          </motion.div>
                        </div>
      ...
      {mobileMode && !backMode &&
        createPortal(
          <AnimatePresence>
            {isMenuOpen && (
              <div key="mobile-root" className="mobile-menu-root" role="dialog" aria-modal="true" aria-labelledby="mobile-menu-heading">
                <motion.div
                  key="mobile-bg"
                  ...
                  onAnimationComplete={(state) => {
                    if (state === 'visible') {
                      clearBgDelay();
                      bgDelayRef.current = window.setTimeout(() => {
                        setBgReady(true);
                        // autofocus op eerste link
                ...
                >
                  <motion.div className="mobile-close-anim" style={{ x: menuX, y: menuY }}>
                    <motion.img src="/images/menu.svg" alt="" aria-hidden="true" className="mobile-close-img"
                      ...
                      transition={{ duration: prefersReduced ? 0 : 0.28, ease: 'easeInOut', opacity: { duration: prefersReduced ? 0 : 0.18 } }}
                    />
                ...
                <motion.div key="mobile-panel" className="mobile-panel" variants={mobilePanelVariants} initial="hidden" animate="visible" exit="exit">
                  <div className="grid-container mobile-panel-inner">
                    <h2 id="mobile-menu-heading" className="sr-only">Primary navigation</h2>
                      ...
                      <div className="cell small-12">
                        <nav id={MENU_ID_MOBILE} className="mobile-panel-nav" aria-label="Primary">
                          <ul className="mobile-list">
                            {menuItems.map((item, index) => {
                              const showHoverDot = hoveredItem === item.label && !item.active;
                              const refProp = index === 0 ? { ref: firstMobileLinkRef } : {};
                              return (
                                <motion.li
                                  key={item.label}
                                  variants={itemVariants}
                                ...
                                >
                                  <a
                                    ...
                                    aria-current={item.active ? 'page' : undefined}
                                  >
                                    <img src={item.icon} alt="" aria-hidden="true" className="mobile-icon" />
                      ...
                      {socialLinks.map((link, index) => (
                        ...
                        >
                          <Magnet padding={20} magnetStrength={10}>
                            ...
                            >
                              <img src={link.icon} alt="" aria-hidden="true" className="mobile-social-img" />
                            </a>
                          </Magnet>
                        </motion.li>
                      ))}
