...
/* ----------------------------- Helpers ----------------------------- */
const cleanToken = (s) => (s || "").replace(/[^\w&-]/g, "").toLowerCase();
const findPhraseStart = (tokens, seq) => {
...
/* ----------------------------- Component ----------------------------- */
const SplitText = ({ text, className = "", delay = 100, duration = 0.6, ease = "power3.out", splitType = "chars", from = { opacity: 0, y: 40 }, to = { opacity: 1, y: 0 }, threshold = 0.6, rootMargin, // not used, kept for API parity
textAlign = "left", lineHeight, wordGap = false, onLetterAnimationComplete, as: Component = "span", startDelay = 0, persistId, groupPhrase, }) => {
    const ref = useRef(null);
    const alreadyPlayed = hasPlayed(persistId);
    useEffect(() => {
        const el = ref.current;
        if (typeof window === "undefined" || !el || !text)
            return;
    ...
    return (_jsx(Component, { ref: ref, className: `split-parent ${className}`, style: style, children: text }));
};
